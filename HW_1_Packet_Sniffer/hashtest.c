...
entry_t *ht_newpair( struct Key key, char *value ) {
	entry_t *newpair;
...
void ht_set( hashtable_t *hashtable, struct Key key, char *value ) {
	int bin = 0;
...
int main( int argc, char **argv ) {
	...
	key[3].key_name="4Key";
	key[3].src_ip="10.10.10.17";
	key[3].dest_ip="11.11.11.1";
	key[3].src_port="232";
	key[3].dest_port="80";

	key[4].key_name="5Key";
	key[4].src_ip="10.10.10.15";
	key[4].dest_ip="11.11.11.1";
	key[4].src_port="232";
	key[4].dest_port="80";

	key[5].key_name="6Key";
	key[5].src_ip="10.10.10.15";
	key[5].dest_ip="11.11.11.1";
	key[5].src_port="232";
	key[5].dest_port="80";
	...
	ht_set( hashtable, key[0], "inkysa" );
	ht_set( hashtable, key[1], "blinkyafqassxss" );
	ht_set( hashtable, key[2], "pinkyfakuk" );
	ht_set( hashtable, key[3], "donkeysaadbd" );
	ht_set( hashtable, key[4], "ergsfafasfgfnf" );
	ht_set( hashtable, key[5], "afssafsafs" );
	...
	//printf("%s \n", ht_get( hashtable, key[0] ));
	     ...
	     for(l=0;l<6;l++){
		for(k=0;k<l;k++){
			if(strcmp(key[k].key_name,key[l].key_name)==0)
				break;}
			if(l==k)
			{printf( "%s key number %s \n", ht_get( hashtable, key[l] ) ,key[l].key_name);}
			}
	return 0;
}
